...
def definir_limites(dificuldade):
    if d := difficulty.lower() if (difficulty := dificuldade) else "":
        ...
        if "avancado" in d:
            return 10, 30
    return 2, 8  # Intermediário


# ==================== MÓDULOS DE FÍSICA (1º ANO) ====================
...
def modulo_estatica_gravitacao(dificuldade="intermediario"):
    ...
    # Clona para garantir mais de 10 variações simuladas no loop para o teste
    while len(questoes) < 10:
        questoes.append(random.choice(banco_estatica))
    return questoes
